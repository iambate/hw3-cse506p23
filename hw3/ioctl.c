...
int pass_to_kernel(int argc, char **argv){
	...
	if (argc == 2) {
		if(strcmp(argv[1], "getall") == 0) {
			...
			if ( (strcmp(argv[1], "get") == 0) && (isNumber(argv[2]) == 0) ) { 
			...
			else if ( (isNumber(argv[1]) == 0) && (isNumber(argv[2]) == 0) ) {
				...
				//perror("Return");
				if (retVal == 999) 
					printf("Process ID already assigned to the Vector Table ID\n");
				else if (retVal != 0)
					printf("Invalid Process ID/Vector Table ID: %d\n",retVal);
				else 
					printf("Process ID %d assigned to the Vector Table ID %d\n",args->process_id, args->vector_table_id);
				free (args);
			}
